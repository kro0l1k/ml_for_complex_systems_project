...
class Config(object):
    ...
    def sample(self, sample_size : int):
        ...
        raw_jump_counts = np.random.poisson(self.jump_intensity * self.terminal_time, size=(sample_size, self.dim_L))
        
        # Cap jump counts to avoid MPS dimension errors
        jump_counts = np.minimum(raw_jump_counts, self.max_jump_count)
        if np.any(raw_jump_counts > self.max_jump_count):
            print(f"Warning: Some samples had more than {self.max_jump_count} jumps. Capping to avoid dimension errors.")
            
        # Use capped jump counts for array dimensions
    ...
    def g(self, x): # - 0.5 * (x - TARGET_MEAN_A)^2
        # Output shape: (batch_size, 1)
        return - 0.5 * torch.sum((x - TARGET_MEAN_A) ** 2, dim=1, keepdim=True)

    def g_x(self, x):
        # Output shape: (batch_size, dim_X)
        return - x + TARGET_MEAN_A
    ...
    def sample_stock_price(self, sample_size, x_0_value=1.0):
        ...
        X = self.X_init.repeat(sample_size, 1) * x_0_value # Shape: (sample_size, dim_X)
...
def main():
    ...
    for x_0 in x_0_values:
        ...
        trajectory = solver.generate_trajectoy(10, x_0_value=x_0)
        # plot the trajectory
